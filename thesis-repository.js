...
// DOM elements
let searchInput, searchButton, departmentFilter, yearFilter, statusFilter, addThesisBtn, thesisTable, pagination;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addThesisBtn = document.querySelector('.add-thesis-btn');
    ...
    // Create modals for add, view, edit
    createModals();
});
...
function setupEventListeners() {
    ...
    addThesisBtn.addEventListener('click', function() {
        ...
        if (modal) {
            // Set Computer Engineering as default department for new theses
            document.getElementById('add-department').value = defaultDepartment;
        ...
        paginatedData.forEach(thesis => {
            ...
            tr.innerHTML = `
                ...
                <td class="action-buttons">
                    ...
                    <button class="edit-btn" title="Edit" data-id="${thesis.id}"><i class="fas fa-edit"></i></button>
                    <button class="download-btn" title="Download" data-id="${thesis.id}"><i class="fas fa-download"></i></button>
                    <button class="delete-btn" title="Delete" data-id="${thesis.id}"><i class="fas fa-trash"></i></button>
                </td>
            `;
            tbody.appendChild(tr);
        });
...
function setupActionButtons() {
    ...
    viewButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (thesis) {
                ...
                modal.querySelector('.thesis-status').className = `thesis-status ${thesis.status}`;
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (thesis) {
                ...
                modal.querySelector('#edit-title').value = thesis.title;
                modal.querySelector('#edit-authors').value = thesis.authors;
                modal.querySelector('#edit-department').value = thesis.department.toLowerCase().replace(/\s+/g, '-');
                modal.querySelector('#edit-status').value = thesis.status;
                modal.querySelector('#edit-abstract').value = thesis.abstract;
                modal.querySelector('#edit-thesis-id').value = thesis.id;
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (thesis && confirm(`Are you sure you want to delete this thesis: "${thesis.title}"?`)) {
                thesisData = thesisData.filter(t => t.id != thesisId);
                filteredData = filteredData.filter(t => t.id != thesisId);
...
function createModals() {
    // Add Thesis Modal
    const addThesisModal = document.createElement('div');
    addThesisModal.id = 'add-thesis-modal';
    addThesisModal.className = 'modal';
    addThesisModal.innerHTML = `
        <div class="modal-content">
            ...
            <form id="add-thesis-form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="add-authors" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea id="add-abstract" rows="5" required></textarea>
                </div>
                <div class="form-group">
                    <label for="add-file">Upload PDF:</label>
                    <input type="file" id="add-file" accept=".pdf" required>
                </div>
    ...
    `;
    document.body.appendChild(addThesisModal);

    // View Thesis Modal
    const viewThesisModal = document.createElement('div');
    viewThesisModal.id = 'view-thesis-modal';
    viewThesisModal.className = 'modal';
    viewThesisModal.innerHTML = `
        <div class="modal-content">
            <span class="close">&times;</span>
            <h2>Thesis Details</h2>
            <div class="thesis-details">
                <h3 class="thesis-title"></h3>
                <p><strong>Authors:</strong> <span class="thesis-authors"></span></p>
                <p><strong>Department:</strong> <span class="thesis-department"></span></p>
                <p><strong>Date Submitted:</strong> <span class="thesis-date"></span></p>
                <p><strong>Status:</strong> <span class="thesis-status"></span></p>
                <p><strong>Abstract:</strong></p>
                <p class="thesis-abstract"></p>
                <div class="thesis-actions">
                    <button class="download-thesis-btn">Download PDF</button>
                </div>
            </div>
        </div>
    `;
    document.body.appendChild(viewThesisModal);

    // Edit Thesis Modal
    const editThesisModal = document.createElement('div');
    editThesisModal.id = 'edit-thesis-modal';
    editThesisModal.className = 'modal';
    editThesisModal.innerHTML = `
        <div class="modal-content">
            <span class="close">&times;</span>
            <h2>Edit Thesis</h2>
            <form id="edit-thesis-form">
                ...
                <div class="form-group">
                    ...
                    <textarea id="edit-abstract" rows="5" required></textarea>
                </div>
                <div class="form-group">
                    <label for="edit-file">Upload New PDF (Optional):</label>
    ...
    `;
    document.body.appendChild(editThesisModal);
...
function setupModalEventListeners() {
    ...
    // Add thesis form submission
    const addThesisForm = document.getElementById('add-thesis-form');
    addThesisForm.addEventListener('submit', function(e) {
        e.preventDefault();
        
        const newThesis = {
            id: thesisData.length > 0 ? Math.max(...thesisData.map(t => t.id)) + 1 : 1,
            title: document.getElementById('add-title').value,
            authors: document.getElementById('add-authors').value,
            department: document.getElementById('add-department').options[document.getElementById('add-department').selectedIndex].text,
            dateSubmitted: new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
            status: document.getElementById('add-status').value,
            abstract: document.getElementById('add-abstract').value,
            pdfUrl: "#" // In a real app, this would be the uploaded file URL
        };
        
        thesisData.unshift(newThesis); // Add to beginning of array
        
        // Re-apply current filters to include new thesis if applicable
        applyFilters();
        
        // Close modal and reset form
        document.getElementById('add-thesis-modal').style.display = 'none';
        addThesisForm.reset();
        
        alert('New thesis added successfully!');
    });

    // Edit thesis form submission
    ...
    editThesisForm.addEventListener('submit', function(e) {
        ...
        const thesisIndex = thesisData.findIndex(t => t.id == thesisId);
        
        if (thesisIndex !== -1) {
            thesisData[thesisIndex] = {
                ...thesisData[thesisIndex],
                title: document.getElementById('edit-title').value,
                authors: document.getElementById('edit-authors').value,
                department: document.getElementById('edit-department').options[document.getElementById('edit-department').selectedIndex].text,
            ...
            // Close modal
            document.getElementById('edit-thesis-modal').style.display = 'none';
            
            alert('Thesis updated successfully!');
        }
    });

    // Download thesis button in view modal
    const downloadThesisBtn = document.querySelector('.download-thesis-btn');
    downloadThesisBtn.addEventListener('click', function() {
        alert('Downloading thesis PDF...');
        // In a real application, this would initiate a download
    });
}